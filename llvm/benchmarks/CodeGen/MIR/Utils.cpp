//===- benchmark/CodeGen/MIR/Utils.cpp --------------------------*- C++ -*-===//
//
...
namespace llvm {
namespace {
/// FIXME: Copy-pasted from llvm/unittests/CodeGen/MFCommon.inc
...
OneFnMIRFixture::OneFnMIRFixture() = default;
OneFnMIRFixture::~OneFnMIRFixture() = default;

void OneFnMIRFixture::SetUp(::benchmark::State &state) {
  assert(!Ctx && !M && !MF);
  Ctx = std::make_unique<LLVMContext>();
  M = std::make_unique<Module>("Module", *Ctx);
  MF = createMachineFunction(*Ctx, *M);

  MRI = &MF->getRegInfo();
}

void OneFnMIRFixture::TearDown(::benchmark::State &state) {
  MRI = nullptr;
